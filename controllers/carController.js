...
const createCar = async (req, res) => {
    const {name, model, engineType, fuelType, mileage, price, imageURL, description} = req.body

    // add doc to db
    try{
        const car = await Car.create({name, model, engineType, fuelType, mileage, price, imageURL, description})
        res.status(200).json(car)
    } catch (error) {
